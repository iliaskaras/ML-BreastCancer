...
class GraphvizCreator(object):
    ...
    def initVariables(self,decisionTreeClf):
        ...
        self.outputFileName = self.decisionTreeClf.get_name() + "_" + self.decisionTreeClf.get_splitFunction() \
                              + "_" + str(self.decisionTreeClf.get_maxDepth()) + ".dot"
        self.initFittedClf()

    def initFittedClf(self):
        self.clf = self.clf.fit(self.decisionTreeClf.x,
                                self.decisionTreeClf.y)
